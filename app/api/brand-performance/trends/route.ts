  ...
  try {
    ...
    const stateSql = `
      ...
      ${commonWhere}
        AND po."status" IN ('DELIVERED', 'COMPLETED')
      GROUP BY b."state"
      ORDER BY SUM(po."amount"::numeric) DESC NULLS LAST
      LIMIT 10;
    `;
    ...
    const orderSizeSql = `
      ...
      FROM (
        SELECT
          CASE
            WHEN po."amount"::numeric <  500   THEN '1 · < ₹500'
            WHEN po."amount"::numeric <  1000  THEN '2 · ₹500–1K'
            WHEN po."amount"::numeric <  2500  THEN '3 · ₹1K–2.5K'
            WHEN po."amount"::numeric <  5000  THEN '4 · ₹2.5K–5K'
            WHEN po."amount"::numeric <  10000 THEN '5 · ₹5K–10K'
            ELSE                                     '6 · > ₹10K'
          END AS bucket,
          po."amount"
        ${fromJoin}
