...
// Additive trends endpoint — surfaces the BA-grade aggregations NOT already
// covered by /trend, /monthly-by-status, /brand-product-summary. Specifically:
...
//   4. kpiDelta   — current window vs immediately-prior comparable window
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // ── 4. KPI grand for current + prior windows ─────────────────────────
    ...
    const [brandMonth, dow, cohort, curGrandRows, prevGrandRows] = await Promise.all([
