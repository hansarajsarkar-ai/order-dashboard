...
// Drill-down behind a single agent row of the "Agent Wise Order" summary table.
// Same scope + last-touch attribution as /agent-orders, but spanning the whole
// selected date range (not a single day). Returns full order detail for each
// attributed PO (same columns as /api/order-list) plus the attributing call's
// duration and recording URL — so the count here reconciles with the table's
// Order Count for that agent.
interface Row {
  poNumber: string;
...
export async function GET(req: NextRequest) {
  ...
  // Scope the call side to the selected range (start_date text compare via buildWhere).
  const f = parseFilters(req);
  const { sql: where, params } = buildWhere(f);

  // Index-friendly IST timestamp bounds for the order side (the whole range).
  ...
  try {
    const rows = await queryNoNestloop<Row>(
      ...
      assigned AS (
        ...
        -- to that buyer on the order's own day (tiebreak: longest call, then agent name).
    ...
    return NextResponse.json({ data, count: data.length, agentName, startDate: f.startDate, endDate: f.endDate });
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err) || 'Unknown error';
    console.error('agent-orders-drill error', err);
