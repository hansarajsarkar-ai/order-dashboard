  ...
  return (
    <div className={`min-h-screen ${t.page} p-8 relative overflow-hidden`}>
      ...
      <div className="w-[95%] mx-auto relative z-10">
        ...
        {bpTab === 'dashboard' && (
        /* Details data — flat un-pivoted table */
        <div className={t.sectionCard}>
          <div className={t.sectionAccent} />
          <div className={t.sectionHeader}>
            <div>
          ...
          <div className={t.tableWrap}>
            {pivotLoading || !pivotData ? (
              ...
              if (sorted.length === 0) {
                return <div className={t.loading}>No rows</div>;
              }
              ...
              return (
                <table className="w-full text-sm">
                  <thead className={`sticky top-0 z-10 ${t.isDark ? 'bg-slate-900/95 backdrop-blur border-b border-white/10' : 'bg-slate-50 border-b border-slate-200'}`}>
                    <tr>
                      <th className={`px-3 py-3 text-left text-[11px] font-semibold uppercase tracking-wider w-12 ${t.isDark ? 'text-purple-200' : 'text-slate-500'}`}>#</th>
                      <th onClick={() => setDetailsSort('brand')}  className={`px-4 py-3 text-left text-[11px] font-semibold uppercase tracking-wider cursor-pointer ${t.isDark ? 'text-purple-200 hover:text-white' : 'text-slate-500 hover:text-slate-900'}`}>Brand{ind('brand')}</th>
                      <th onClick={() => setDetailsSort('month')}  className={`px-3 py-3 text-left text-[11px] font-semibold uppercase tracking-wider cursor-pointer ${t.isDark ? 'text-purple-200 hover:text-white' : 'text-slate-500 hover:text-slate-900'}`}>Month{ind('month')}</th>
                      <th onClick={() => setDetailsSort('status')} className={`px-3 py-3 text-left text-[11px] font-semibold uppercase tracking-wider cursor-pointer ${t.isDark ? 'text-purple-200 hover:text-white' : 'text-slate-500 hover:text-slate-900'}`}>Status{ind('status')}</th>
                      <th className={`px-3 py-3 text-left text-[11px] font-semibold uppercase tracking-wider ${t.isDark ? 'text-purple-200' : 'text-slate-500'}`}>Delivery Status</th>
                      <th onClick={() => setDetailsSort('orders')} className={`px-3 py-3 text-right text-[11px] font-semibold uppercase tracking-wider cursor-pointer ${t.isDark ? 'text-purple-200 hover:text-white' : 'text-slate-500 hover:text-slate-900'}`}>Orders{ind('orders')}</th>
                      <th onClick={() => setDetailsSort('gmv')}    className={`px-3 py-3 text-right text-[11px] font-semibold uppercase tracking-wider cursor-pointer ${t.isDark ? 'text-purple-200 hover:text-white' : 'text-slate-500 hover:text-slate-900'}`}>GMV{ind('gmv')}</th>
                    </tr>
                  </thead>
                  <tbody>
                    {sorted.map((r, i) => (
                      <tr key={`${r.brandName}__${r.month}__${r.status}__${r.deliveryStatus ?? '_'}__${i}`} className={`${t.isDark ? 'border-b border-white/5' : 'border-b border-slate-200'} ${t.rowHover} ${i % 2 === 0 ? t.rowEven : t.rowOdd}`}>
                        <td className={`px-3 py-2.5 tabular-nums w-12 ${t.isDark ? 'text-purple-300/60' : 'text-slate-400'}`}>{i + 1}</td>
                        <td className="px-4 py-2.5 whitespace-nowrap">
                          <div className="flex items-center gap-2">
                            <span className={t.brandAccent} />
                            <span className={t.brandName}>{r.brandName}</span>
                          </div>
                        </td>
                        <td className={`px-3 py-2.5 whitespace-nowrap ${t.isDark ? 'text-purple-200' : 'text-slate-600'}`}>{MONTH_NAMES[r.month - 1] || r.month}</td>
                        <td className="px-3 py-2.5 whitespace-nowrap">
                          <span className={`inline-block px-2 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-wider border ${t.pillBgFor(r.status)}`}>
                            {r.status}
                          </span>
                        </td>
                        <td className={`px-3 py-2.5 whitespace-nowrap ${t.isDark ? 'text-purple-200' : 'text-slate-600'}`}>{r.deliveryStatus || <span className={t.isDark ? 'italic text-purple-400/60' : 'italic text-slate-400'}>(none)</span>}</td>
                        <td className={`px-3 py-2.5 text-right text-base font-extrabold tabular-nums ${t.isDark ? 'text-white' : 'text-slate-900'}`}>{r.count.toLocaleString('en-IN')}</td>
                        <td className={`px-3 py-2.5 text-right text-sm font-semibold tabular-nums ${t.isDark ? 'text-purple-300/80' : 'text-slate-600'}`}>{formatAmount(r.amount)}</td>
                      </tr>
                    ))}
                  </tbody>
                  <tfoot className={t.footRow}>
                    <tr>
                      ...
                      <td className={`px-4 py-3 font-bold uppercase text-[11px] tracking-wider ${t.isDark ? 'text-purple-200' : 'text-slate-700'}`}>Filtered total</td>
                      <td className={`px-3 py-3 text-xs ${t.isDark ? 'text-purple-300/70' : 'text-slate-500'}`}>{filtered.length} rows</td>
                      <td className="px-3 py-3" />
                      <td className="px-3 py-3" />
                      <td className={`px-3 py-3 text-right text-base font-extrabold tabular-nums ${t.isDark ? 'text-white' : 'text-slate-900'}`}>{totalOrders.toLocaleString('en-IN')}</td>
                      <td className={`px-3 py-3 text-right text-sm font-semibold tabular-nums ${t.isDark ? 'text-fuchsia-200' : 'text-purple-700'}`}>{formatAmount(totalGmv)}</td>
                    </tr>
                  </tfoot>
                </table>
              );
