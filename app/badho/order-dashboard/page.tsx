...
export default function OrderStatusDashboard() {
  ...
  // Monthly Geo Coverage by Status — rows = order status, geo toggle drives the "Covered" column
  interface GeoCovered { pincode: number; city: number; district: number; state: number }
  interface GeoStatusCell { covered: GeoCovered; orders: number; amount: number }
  interface GeoStatusData {
    data: { status: string; months: Record<number, GeoStatusCell>; total: GeoStatusCell }[];
    totals: { byMonth: Record<number, GeoStatusCell>; grand: GeoStatusCell };
    year: number;
  }
  const [geoStatusData, setGeoStatusData] = useState<GeoStatusData | null>(null);
  const [geoStatusLoading, setGeoStatusLoading] = useState(true);
  const [geoStatusLevel, setGeoStatusLevel] = useState<GeoKey>('pincode');
  ...
  const fetchGeoStatus = async () => {
    try {
      ...
      const response = await fetch(`/api/order-monthly-geo-coverage-by-status?year=${currentYear}`);
      if (!response.ok) throw new Error('Failed to fetch geo coverage by status');
      const result: GeoStatusData = await response.json();
      setGeoStatusData(result);
    } catch (err) {
      console.error('Geo status fetch error:', err);
    } finally {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 p-8 relative overflow-hidden">
      ...
      <div className="w-[95%] mx-auto relative z-10">
        ...
        {/* Monthly Geo Coverage by Order Status — rows = status, geo toggle drives Covered column */}
        <div className="mt-8 bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl overflow-hidden transition-all duration-300 hover:bg-white/10 hover:border-fuchsia-400/50 hover:shadow-[0_0_50px_rgba(217,70,239,0.25),inset_0_0_30px_rgba(168,85,247,0.12)]">
          <div className="px-8 py-6 border-b border-white/10 bg-white/5 flex items-center justify-between flex-wrap gap-4">
            <div>
              <h2 className="text-2xl font-bold text-white">Monthly Geo Coverage by Order Status</h2>
              <p className="text-purple-300 text-sm mt-1">
                Distinct <span className="font-semibold text-fuchsia-300">{geoStatusLevel}</span>s covered per status &amp; month, with order count and value — {currentYear}
              </p>
            </div>
            <div className="flex items-center gap-3 flex-wrap">
              {/* Geo level toggle */}
              <div className="flex gap-1 p-1 bg-white/5 border border-white/10 rounded-xl">
                {(['pincode', 'city', 'district', 'state'] as GeoKey[]).map((g) => {
                  const active = geoStatusLevel === g;
                  return (
                    <button
                      key={g}
                      onClick={() => setGeoStatusLevel(g)}
              ...
              {geoStatusData && (
                <button
                  className={DOWNLOAD_BTN_CLASS}
                  onClick={() => {
                    const headers = ['Status', ...MONTH_NAMES.flatMap((m) => [`${m} ${geoStatusLevel} covered`, `${m} Orders`, `${m} Value`]), `Total ${geoStatusLevel} covered`, 'Total Orders', 'Total Value'];
                    const rows: CsvCell[][] = geoStatusData.data.map((row) => {
                      const monthCells = MONTH_NAMES.flatMap((_, i) => {
                        const c = row.months[i + 1];
                        return [c?.covered[geoStatusLevel] ?? 0, c?.orders ?? 0, c?.amount ?? 0];
                      });
                      return [row.status, ...monthCells, row.total.covered[geoStatusLevel], row.total.orders, row.total.amount];
                    });
                    downloadCSV(`monthly-geo-coverage-by-status-${geoStatusLevel}-${currentYear}.csv`, headers, rows);
                  }}
                >
                  ↓ CSV
                </button>
          ...
          <div className="overflow-x-auto">
            {geoStatusLoading ? (
              <div className="px-8 py-12 text-center">
                <div className="flex flex-col items-center gap-3">
                  <div className="w-8 h-8 rounded-full border-2 border-fuchsia-500/30 border-t-fuchsia-500 animate-spin" />
                  <p className="text-purple-300">Loading geo coverage by status…</p>
                </div>
              </div>
            ) : !geoStatusData || geoStatusData.data.length === 0 ? (
              <div className="px-8 py-12 text-center text-purple-300">No data available</div>
            ) : (
              <table className="w-full text-sm">
                <thead>
                  ...
                  <tr className="bg-white/5 border-b border-white/10">
                    {[...Array(13)].map((_, i) => {
                      ...
                      return (
                        <Fragment key={i}>
                          <th className={`px-2 py-2 text-right text-[10px] font-medium capitalize ${isTotal ? 'text-purple-100 bg-purple-500/20' : 'text-purple-300'}`}>{geoStatusLevel}</th>
                          <th className={`px-2 py-2 text-right text-[10px] font-medium ${isTotal ? 'text-purple-100 bg-purple-500/20' : 'text-purple-300'}`}>Orders</th>
                          <th className={`px-2 py-2 text-right text-[10px] font-medium border-r border-white/10 ${isTotal ? 'text-purple-100 bg-purple-500/20' : 'text-purple-300'}`}>Value</th>
                        </Fragment>
                      );
                ...
                <tbody>
                  {geoStatusData.data.map((row) => (
                    <tr key={row.status} className="border-b border-white/5 hover:bg-fuchsia-500/10 transition-colors">
                      <td className="px-4 py-3 sticky left-0 bg-slate-900/80 backdrop-blur z-10 border-r border-white/10 text-white text-sm font-semibold">
                        {row.status}
                      </td>
                      {MONTH_NAMES.map((_, idx) => {
                        const month = idx + 1;
                        const cell = row.months[month];
                        const hasData = cell && cell.orders > 0;
                        return (
                          <Fragment key={month}>
                            <td className={`px-2 py-3 text-right tabular-nums ${hasData ? 'text-fuchsia-200 font-semibold' : 'text-white/30'}`}>
                              {hasData ? cell.covered[geoStatusLevel].toLocaleString() : '—'}
                            </td>
                            <td className={`px-2 py-3 text-right tabular-nums ${hasData ? 'text-white' : 'text-white/30'}`}>
                              {hasData ? cell.orders.toLocaleString() : '—'}
                            </td>
                            <td className={`px-2 py-3 text-right tabular-nums border-r border-white/10 ${hasData ? 'text-purple-200' : 'text-white/30'}`}>
                              {hasData ? formatAmount(cell.amount) : '—'}
                            </td>
                      ...
                      <td className="px-2 py-3 text-right tabular-nums font-bold text-fuchsia-100 bg-purple-500/10">
                        {row.total.covered[geoStatusLevel].toLocaleString()}
                      </td>
                      <td className="px-2 py-3 text-right tabular-nums font-bold text-white bg-purple-500/10">
                        {row.total.orders.toLocaleString()}
                      </td>
                      <td className="px-2 py-3 text-right tabular-nums font-bold text-purple-100 bg-purple-500/10 border-r border-white/10">
                        {formatAmount(row.total.amount)}
                      </td>
                  ...
                  <tr className="bg-gradient-to-r from-purple-500/20 to-blue-500/20 border-t-2 border-purple-400/40 font-bold">
                    ...
                    {MONTH_NAMES.map((_, idx) => {
                      const month = idx + 1;
                      const cell = geoStatusData.totals.byMonth[month];
                      const hasData = cell && cell.orders > 0;
                      return (
                        <Fragment key={month}>
                          <td className={`px-2 py-3 text-right tabular-nums ${hasData ? 'text-fuchsia-100' : 'text-white/30'}`}>
                            {hasData ? cell.covered[geoStatusLevel].toLocaleString() : '—'}
                          </td>
                          <td className={`px-2 py-3 text-right tabular-nums ${hasData ? 'text-white' : 'text-white/30'}`}>
                            {hasData ? cell.orders.toLocaleString() : '—'}
                    ...
                    <td className="px-2 py-3 text-right tabular-nums text-fuchsia-50 bg-purple-500/30">
                      {geoStatusData.totals.grand.covered[geoStatusLevel].toLocaleString()}
                    </td>
                    <td className="px-2 py-3 text-right tabular-nums text-white bg-purple-500/30">
                      {geoStatusData.totals.grand.orders.toLocaleString()}
                    </td>
                    <td className="px-2 py-3 text-right tabular-nums text-purple-50 bg-purple-500/30 border-r border-white/10">
                      {formatAmount(geoStatusData.totals.grand.amount)}
                    </td>
                  </tr>
                </tbody>
              </table>
