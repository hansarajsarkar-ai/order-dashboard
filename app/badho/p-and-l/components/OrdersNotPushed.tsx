...
// Orders-not-pushed have no delivery leg yet → no Expected Delivery Loss
// dimension here; only the three discount % dimensions the query produces.
...
export default function OrdersNotPushed() {
  ...
  useEffect(() => {
    let alive = true;
    setLoading(true);
    fetch('/api/p-and-l/orders-not-pushed')
      .then((r) => r.json())
      .then((d) => {
        if (!alive) return;
        if (d.error) {
  ...
  const kpis = useMemo(() => {
    ...
    for (const r of rows) {
      ...
      itemDisc += numOf(r['ItemDiscount']);
      coupon += numOf(r['CoupanApplied']);
    }
    return { count: rows.length, gross, itemDisc, coupon };
  }, [rows]);
  ...
  const displayRows = useMemo(() => {
    ...
    const seller = filterSeller.trim().toLowerCase();
    let out = rows.filter((r) => {
      if (po && !String(r['poNumber'] ?? '').toLowerCase().includes(po)) return false;
      if (seller && !String(r['sellerbusinessname'] ?? '').toLowerCase().includes(seller)) return false;
      return true;
    });
    if (sortKey) {
  ...
  if (loading) {
    return (
      <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-12 text-center">
        <div className="text-purple-200 text-sm animate-pulse">Loading orders not pushed…</div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <Kpi label="Orders Not Pushed" value={kpis.count.toLocaleString('en-IN')} accent="from-fuchsia-500/30 to-purple-600/30" />
        <Kpi label="Gross Amount" value={fmtINR(kpis.gross)} accent="from-indigo-500/30 to-blue-600/30" />
        <Kpi label="Total Item Discount" value={fmtINR(kpis.itemDisc)} accent="from-amber-500/30 to-orange-600/30" valueClass="text-amber-200" />
        <Kpi label="Total Coupon Applied" value={fmtINR(kpis.coupon)} accent="from-rose-500/30 to-fuchsia-600/30" valueClass="text-rose-200" />
      </div>
                      ...
                      <button
                        onClick={() =>
                          openModal(
                            ...
                            `Orders not pushed matching this slab — sorted by ${dim.title}`,
                            br,
                            dim.key
                          )
                        }
      ...
      <div className="rounded-2xl border border-white/10 bg-white/[0.03] backdrop-blur-xl overflow-hidden">
        <div className="flex items-center justify-between gap-3 px-5 py-3 border-b border-white/10 flex-wrap">
          <div className="flex items-center gap-3 flex-wrap">
            <h3 className="text-sm font-bold text-white">Orders Not Pushed</h3>
            {/* Filters */}
        ...
        <div className="overflow-auto max-h-[70vh]">
          {displayRows.length === 0 ? (
            <div className="p-12 text-center text-purple-300/70 text-sm">
              {rows.length === 0 ? 'No orders awaiting push.' : 'No rows match the current filters.'}
            </div>
          ) : (
